...
- `scripts/riot_champion_build_wiki_sync.py` — syncs a mechanically ranked compact block from one explicitly selected item/build `analysis.json` into champion entity pages while retaining full detail under `reports/`. Its rankings are report candidates, not qualitative curation; the automatic invocation from `lint.py` is mechanical synchronization only. When explicitly using it, run `--dry-run` first, use `--write` only for the intended snapshot, and finish with `--check`; do not edit generated blocks by hand.
- `scripts/riot_champion_matchup_wiki_sync.py` — aggregates ally-combo and same-role opponent candidates, and can sync up to three high-win-rate combos and three low-target-win-rate counter-pick examples per role from an existing `analysis.json` with `--analysis`. Its results are descriptive; use `--dry-run` before `--write`, finish with `--check`, and do not treat the generated block as causal curation.
- `scripts/riot_champion_duration_wiki_sync.py` — syncs the latest existing champion-by-duration result into the `power-spike-match` block. It preserves the detailed report under `reports/`, supports `--dry-run`, `--write`, and `--check`, and does not interpret post-match duration as a causal power spike.
...
- `scripts/riot_champion_tier_wiki_sync.py` — syncs a compact observed-tier champion block (per-tier pick-count top candidates plus high/low win-rate exploratory candidates) from one explicitly selected tier-analysis `analysis.json` into the corresponding champion entities by `champion_key`. Run `--dry-run` before `--write` and finish with `--check`; the block is descriptive, not a recommendation.
- `scripts/lint.py` — selects the newest completed run independently for duration/power-spike, build, matchup, rune, and observed-tier reports using manifest outputs and `generated_at`, then runs dry-run, write, and check for the entity synchronizers before the static lint checks. Use `--check-only` when writes are not allowed; report candidates remain descriptive and require separate qualitative curation.
...
Running `scripts/lint.py` alone does not complete report curation; it now refreshes mechanically generated entity blocks from the latest completed reports, while qualitative selection and evidence review remain part of the agent's lint workflow. Rankings or generated blocks from report and sync scripts do not count as curated findings by themselves.
