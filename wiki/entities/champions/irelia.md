...
- **高勝率コンボ候補（最大3件）：**
  - [[wiki/entities/champions/ashe|アッシュ（Ashe）]] — 対象側勝率63.5%（33/52）、n=52（十分性の目安を満たす）。
  - [[wiki/entities/champions/lillia|リリア（Lillia）]] — 対象側勝率60.6%（20/33）、n=33（十分性の目安を満たす）。
  - [[wiki/entities/champions/jinx|ジンクス（Jinx）]] — 対象側勝率52.9%（18/34）、n=34（十分性の目安を満たす）。
- **低勝率カウンターピック候補（最大3件）：**
  - [[wiki/entities/champions/garen|ガレン（Garen）]] — 対象側勝率40.0%（12/30）、n=30（十分性の目安を満たす）。
  - [[wiki/entities/champions/yorick|ヨリック（Yorick）]] — 対象側勝率47.7%（21/44）、n=44（十分性の目安を満たす）。
  - [[wiki/entities/champions/mordekaiser|モルデカイザー（Mordekaiser）]] — 対象側勝率50.0%（15/30）、n=30（十分性の目安を満たす）。

### MIDDLE（対象n=535）

- **高勝率コンボ候補（最大3件）：**
  - [[wiki/entities/champions/yunara|ユナラ（Yunara）]] — 対象側勝率67.7%（21/31）、n=31（十分性の目安を満たす）。
  - [[wiki/entities/champions/miss-fortune|ミス・フォーチュン（MissFortune）]] — 対象側勝率62.5%（25/40）、n=40（十分性の目安を満たす）。
  - [[wiki/entities/champions/mordekaiser|モルデカイザー（Mordekaiser）]] — 対象側勝率59.6%（31/52）、n=52（十分性の目安を満たす）。
- **低勝率カウンターピック候補（最大3件）：**
  - [[wiki/entities/champions/yone|ヨネ（Yone）]] — 対象側勝率45.0%（18/40）、n=40（十分性の目安を満たす）。
...
### TOP（対象315試合、全体勝率47.3%）

| 順位 | 完全ルーンセット | 選択数 | 選択率 | 選択時勝率 |
| ---: | --- | ---: | ---: | ---: |
| 1 | 主系栄華：征服者／凱旋・レジェンド: 迅速・背水の陣；副系天啓：トリプル トニック・なんでも屋；シャードUNKNOWN(5005)・UNKNOWN(5008)・UNKNOWN(5011) | 50/315 | 15.9% | 58.0% |
| 2 | 主系栄華：征服者／凱旋・レジェンド: 迅速・背水の陣；副系天啓：ビスケットデリバリー・なんでも屋；シャードUNKNOWN(5005)・UNKNOWN(5008)・UNKNOWN(5001) | 46/315 | 14.6% | 43.5% |
| 3 | 主系栄華：征服者／凱旋・レジェンド: 迅速・背水の陣；副系天啓：なんでも屋・トリプル トニック；シャードUNKNOWN(5005)・UNKNOWN(5008)・UNKNOWN(5011) | 38/315 | 12.1% | 52.6% |
