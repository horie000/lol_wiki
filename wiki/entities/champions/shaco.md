...
- **高勝率コンボ候補（最大3件）：**
  - [[wiki/entities/champions/yone|ヨネ（Yone）]] — 対象側勝率61.8%（21/34）、n=34（十分性の目安を満たす）。
  - [[wiki/entities/champions/viktor|ビクター（Viktor）]] — 対象側勝率59.4%（19/32）、n=32（十分性の目安を満たす）。
  - [[wiki/entities/champions/jhin|ジン（Jhin）]] — 対象側勝率55.8%（29/52）、n=52（十分性の目安を満たす）。
- **低勝率カウンターピック候補（最大3件）：**
  - [[wiki/entities/champions/nautilus|ノーチラス（Nautilus）]] — 対象側勝率44.1%（15/34）、n=34（十分性の目安を満たす）。
  - [[wiki/entities/champions/leona|レオナ（Leona）]] — 対象側勝率46.7%（21/45）、n=45（十分性の目安を満たす）。
  - [[wiki/entities/champions/thresh|スレッシュ（Thresh）]] — 対象側勝率48.4%（15/31）、n=31（十分性の目安を満たす）。

### JUNGLE（対象n=451）

- **高勝率コンボ候補（最大3件）：**
  - [[wiki/entities/champions/jhin|ジン（Jhin）]] — 対象側勝率58.8%（20/34）、n=34（十分性の目安を満たす）。
  - [[wiki/entities/champions/ashe|アッシュ（Ashe）]] — 対象側勝率58.1%（18/31）、n=31（十分性の目安を満たす）。
  - [[wiki/entities/champions/nautilus|ノーチラス（Nautilus）]] — 対象側勝率51.3%（20/39）、n=39（十分性の目安を満たす）。
- **低勝率カウンターピック候補（最大3件）：**
...
### UTILITY（対象319試合、全体勝率52.7%）

| 順位 | 完全ルーンセット | 選択数 | 選択率 | 選択時勝率 |
| ---: | --- | ---: | ---: | ---: |
| 1 | 主系覇道：ヘイルブレード／サドンインパクト・第六感・執拗な賞金首狩り；副系天啓：魔法の靴・宇宙の英知；シャードUNKNOWN(5008)・UNKNOWN(5008)・UNKNOWN(5001) | 84/319 | 26.3% | 59.5% |
| 2 | 主系覇道：ヘイルブレード／サドンインパクト・第六感・執拗な賞金首狩り；副系天啓：魔法の靴・宇宙の英知；シャードUNKNOWN(5005)・UNKNOWN(5008)・UNKNOWN(5001) | 72/319 | 22.6% | 56.9% |
| 3 | 主系覇道：ヘイルブレード／サドンインパクト・第六感・執拗な賞金首狩り；副系天啓：魔法の靴・宇宙の英知；シャードUNKNOWN(5005)・UNKNOWN(5008)・UNKNOWN(5011) | 45/319 | 14.1% | 40.0% |
...
| --- | ---: | ---: | ---: | --- | ---: |
| PLATINUM | 1 | 266 | 2.7% | 143勝/123敗 | 53.8% |

### 高勝率候補（上位5）

| 観測帯 | 候補順位 | 試合数 | 勝敗 | 勝率 |
| --- | ---: | ---: | --- | ---: |
| BRONZE | 4 | 28 | 19勝/9敗 | 67.9% |
| DIAMOND | 3 | 44 | 28勝/16敗 | 63.6% |
